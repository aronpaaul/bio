import { fileURLToPath } from 'node:url'
import { dirname, join } from 'node:path'

const here = dirname(fileURLToPath(import.meta.url))

export const host = '127.0.0.1'
export const port = Number(process.env.PORT) || 3001
export const distDir = join(here, '..', 'dist')
export const dataFile = join(here, 'data', 'guestbook.json')
export const visitFile = join(here, 'data', 'visits.json')
export const logFile = join(here, 'data', 'guestbook.log')
export const challengeTtl = 30000
export const visitWindow = 21600000
export const maxNameLength = 32
export const maxMessageLength = 280
export const perPage = 3
export const bodyLimit = '8kb'
export const allowedOrigins = (process.env.ALLOWED_ORIGINS ?? 'http://localhost:5173,http://127.0.0.1:5173,http://localhost:5185,https://paul.rip').split(',')
export const botAgents = /(python-requests|curl\/|wget|go-http|okhttp|scrapy|libwww|httpclient|java\/|node-fetch|axios|postman|bot\b)/i
export const likeSecret = process.env.GUESTBOOK_SECRET ?? 'dev-secret-paul-1999'
